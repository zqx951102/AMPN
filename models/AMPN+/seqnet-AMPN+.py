...
#from models.hoim import HOIMLoss
...
class SeqNet(nn.Module):
    def __init__(self, cfg):
        ...
        backbone, box_head = build_resnet(name="seresnet50", pretrained=True)
        ...
        # loss weights
        self.lw_rpn_reg = cfg.SOLVER.LW_RPN_REG #1  Loss weight of RPN regression
        self.lw_rpn_cls = cfg.SOLVER.LW_RPN_CLS#1  Loss weight of RPN classification
        self.lw_proposal_reg = cfg.SOLVER.LW_PROPOSAL_REG #Loss weight of proposal regression 10
        self.lw_proposal_cls = cfg.SOLVER.LW_PROPOSAL_CLS #Loss weight of proposal classification 1
        self.lw_box_reg = cfg.SOLVER.LW_BOX_REG #Loss weight of box regression 1
        self.lw_box_cls = cfg.SOLVER.LW_BOX_CLS #Loss weight of box classification 1
        self.lw_box_reid = cfg.SOLVER.LW_BOX_REID #Loss weight of box OIM (i.e. Online Instance Matching) 1
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        query_img_as_gallery: Set to True to detect all people in the query image.
            ...
            This option serves CBGM. gt
        """
        original_image_sizes = [img.shape[-2:] for img in images]
        images, targets = self.transform(images, targets)
        features = self.backbone(images.tensors)

        if query_img_as_gallery:  #Set to True to detect all people in the query image.
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        losses["loss_proposal_reg"] *= self.lw_proposal_reg #Loss weight of proposal regression 10
        losses["loss_proposal_cls"] *= self.lw_proposal_cls #Loss weight of proposal classification 1
        losses["loss_box_reg"] *= self.lw_box_reg #Loss weight of box regression 1
        losses["loss_box_cls"] *= self.lw_box_cls #Loss weight of box classification 1
        losses["loss_box_reid"] *= self.lw_box_reid #Loss weight of box OIM (i.e. Online Instance Matching) 1
        return losses
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        # ------------------- Faster R-CNN head ------------------ #
        proposal_features = self.box_roi_pool(features, proposals, image_shapes) #MultiScaleRoIAlign
        ...
        if self.training:
            ...
            # invoke the postprocess method inherited from parent class to process proposals调用从父类继承的postprocess方法来处理提议
            boxes, scores, _ = self.postprocess_proposals(
                proposal_cls_scores, proposal_regs, proposals, image_shapes
            )

        cws = True  #Confidence Weighted Similarity (CWS)
        gt_det = None
        ...
        if self.training:
            ...
            # set to original thresh after finishing postprocess 后处理
    ...
    def postprocess_boxes( #定义 postprocess_boxes
        self,
        class_logits,
        box_regression,
        embeddings,
